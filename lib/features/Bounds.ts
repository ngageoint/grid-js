...
export class Bounds extends GeometryEnvelope {
	 ...
	 * @return bounds
	 */
	public static degrees(minLongitude: number, minLatitude: number,
		maxLongitude: number, maxLatitude: number): Bounds {
		return this.bounds(minLongitude, minLatitude, maxLongitude, maxLatitude,
			Unit.DEGREE);
	}
	 ...
	 * @return bounds
	 */
	public static meters(minLongitude: number, minLatitude: number,
		maxLongitude: number, maxLatitude: number): Bounds {
		return this.bounds(minLongitude, minLatitude, maxLongitude, maxLatitude,
			Unit.METER);
	}
	...
	public toPrecision(precision: number): Bounds {
		...
		const minLon = GridUtils.precisionBefore(bounds.getMinLongitude(),
			precision);
		const minLat = GridUtils.precisionBefore(bounds.getMinLatitude(),
			precision);
		const maxLon = GridUtils.precisionAfter(bounds.getMaxLongitude(),
			precision);
		const maxLat = GridUtils.precisionAfter(bounds.getMaxLatitude(),
			precision);
	...
	public getPixelRange(width: number, height: number, bounds: Bounds): PixelRange {
		...
		const topLeft = GridUtils.getPixel(width, height, bounds,
			this.getNorthwest());
		const bottomRight = GridUtils.getPixel(width, height, bounds,
			this.getSoutheast());
